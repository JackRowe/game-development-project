...
func animatePropeller(delta: float) -> void:
	...
	var rotation = (parent.thrustValue * propellerMultiplier) + propellerIdle
...
func animateElevator(_delta: float) -> void:
	...
	var rotation = Basis.from_euler(Vector3(Physics.GetControlSurfaceAngle("Elevator", parent.pitchValue, true), 0, 0))
...
func animateRudder(_delta: float) -> void:
	...
	var rotation = Basis.from_euler(Vector3(0, 0, Physics.GetControlSurfaceAngle("Rudder", parent.yawValue, true)))
